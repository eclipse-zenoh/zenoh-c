...
typedef struct z_info_inner_t z_info_inner_t;
...
typedef struct z_owned_session_t {
  struct z_session_t *borrow;
} z_owned_session_t;

typedef struct z_owned_config_t {
  struct z_config_t *borrow;
} z_owned_config_t;
 ...
 *
 */
typedef struct z_hello_array_t {
  const struct zn_hello_t *val;
  size_t len;
} z_hello_array_t;

typedef struct z_info_t {
  struct z_info_inner_t *borrow;
} z_info_t;
 ...
 *
 */
typedef struct z_reply_data_t {
  struct z_sample_t data;
 ...
 *
 */
typedef struct z_reply_t {
  enum z_reply_t_Tag tag;
  struct z_reply_data_t data;
} z_reply_t;
 ...
 *
 */
typedef struct z_reply_data_array_t {
 ...
 *    b : The array to free.
 */
void z_bytes__free(struct z_bytes_t b);
 ...
 */
struct z_owned_config_t z_config__client(char *peer);

/**
 * Create a default set of properties for zenoh-net session configuration.
 */
struct z_owned_config_t z_config__default(void);

/**
 * Create an empty set of properties for zenoh-net session configuration.
 */
struct z_owned_config_t z_config__empty(void);
 ...
 */
void z_config__free(struct z_owned_config_t ps);
 ...
 */
struct z_owned_config_t z_config__from_file(const char *path);
 ...
 */
struct z_owned_config_t z_config__from_str(const char *s);
 ...
 */
struct z_string_t z_config__get(const struct z_config_t *ps, unsigned int key);
 ...
 */
void z_config__insert(struct z_config_t *ps, unsigned long key, struct z_string_t value);
 ...
 */
unsigned int z_config__len(const struct z_config_t *ps);

/**
 * Return a new empty configuration.
 */
struct z_owned_config_t z_config__new(void);

/**
 * Create a default set of properties for peer mode zenoh-net session configuration.
 */
struct z_owned_config_t z_config__peer(void);
 ...
 */
struct z_string_t z_config__to_str(const struct z_config_t *config);
 ...
 *
 */
void z_hello_array__free(struct z_hello_array_t hellos);
 ...
 */
struct z_info_t z_info(const struct z_session_t *session);

void z_info__free(struct z_info_t info);

struct z_string_t z_info__get(const struct z_info_inner_t *info, uint64_t key);
 ...
 */
void z_pull(struct z_subscriber_t *sub);
...
void z_query(struct z_session_t *session,
             ...
             void (*callback)(struct z_reply_t, const void*),
             void *arg);
 ...
 */
struct z_string_t z_query__predicate(const struct z_query_t *query);
 ...
 */
struct z_string_t z_query__res_name(const struct z_query_t *query);
 ...
 */
struct z_query_consolidation_t z_query_consolidation__default(void);

/**
 * Create a default :c:type:`zn_query_target_t`.
 */
struct z_query_target_t z_query_target__default(void);
...
void z_reply__free(struct z_reply_t reply);
 ...
 *
 */
void z_reply_data__free(struct z_reply_data_t reply_data);
 ...
 *
 */
void z_sample__free(struct z_sample_t sample);
 ...
 *     An array of :c:struct:`zn_hello_t` messages.
 */
struct z_hello_array_t z_scout(unsigned int what,
 ...
 *
 */
void z_str_array__free(struct z_str_array_t strs);

/**
 * Frees the passed z_string_t.
 */
void z_string__free(struct z_string_t s);
 ...
 */
struct z_string_t z_string__new(const char *s);

/**
 * Create a default subscription info.
 */
struct z_subinfo_t z_subinfo__default(void);

const struct z_period_t *z_subinfo__period(const struct z_subinfo_t *info);

/**
 * Create a default :c:type:`zn_target_t`.
 */
struct z_target_t z_target__default(void);
 ...
 */
struct z_string_t zn_info_as_str(const struct z_session_t *session);
