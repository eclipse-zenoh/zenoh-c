                ...
                if let Err(e) = sub
                    ...
                    .fetch({
                        ...
                        move |cb| match options {
                            Some(options) => {
                                let mut get_builder = s
                                    .get(selector)
                                    .accept_replies(options.query_accept_replies.into())
                                    .consolidation(options.query_consolidation);

                                if options.query_timeout_ms != 0 {
                                    get_builder = get_builder.timeout(
                                        std::time::Duration::from_millis(options.query_timeout_ms),
                                    );
                                }
                                get_builder.callback(cb).res_sync()
                            }
