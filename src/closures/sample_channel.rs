...
#[no_mangle]
pub extern "C" fn z_internal_fifo_handler_sample_check(this_: &z_owned_fifo_handler_sample_t) -> bool {
...
/// Returns ``true`` if handler is valid, ``false`` if it is in gravestone state.
#[no_mangle]
pub extern "C" fn z_internal_ring_handler_sample_check(this_: &z_owned_ring_handler_sample_t) -> bool {
    this_.as_rust_type_ref().is_some()
}
