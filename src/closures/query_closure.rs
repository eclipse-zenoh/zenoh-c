...
pub unsafe extern "C" fn _z_closure_query_null(this_: *mut MaybeUninit<z_owned_closure_query_t>) {
    (*this_).write(z_owned_closure_query_t::default());
}

/// Returns ``true`` if closure is valid, ``false`` if it is in gravestone state.
#[no_mangle]
pub extern "C" fn _z_closure_query_check(this_: &z_owned_closure_query_t) -> bool {
    !this_.is_empty()
}
