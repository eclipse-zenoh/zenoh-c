...
#[prebindgen]
pub fn z_internal_fifo_handler_query_null(this_: &mut MaybeUninit<z_owned_fifo_handler_query_t>) {
    this_.as_rust_type_mut_uninit().write(None);
}

/// Returns ``true`` if handler is valid, ``false`` if it is in gravestone state.
#[prebindgen]
pub fn z_internal_fifo_handler_query_check(this_: &z_owned_fifo_handler_query_t) -> bool {
...
/// Constructs a handler in gravestone state.
#[prebindgen]
pub fn z_internal_ring_handler_query_null(this_: &mut MaybeUninit<z_owned_ring_handler_query_t>) {
    this_.as_rust_type_mut_uninit().write(None);
}

/// Returns ``true`` if handler is valid, ``false`` if it is in gravestone state.
#[prebindgen]
pub fn z_internal_ring_handler_query_check(this_: &z_owned_ring_handler_query_t) -> bool {
    this_.as_rust_type_ref().is_some()
}
