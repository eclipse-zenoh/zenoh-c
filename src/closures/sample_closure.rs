...
pub unsafe extern "C" fn _z_closure_sample_null(this_: &mut MaybeUninit<z_owned_closure_sample_t>) {
    this_.write(z_owned_closure_sample_t::default());
}

/// Returns ``true`` if closure is valid, ``false`` if it is in gravestone state.
#[no_mangle]
pub extern "C" fn _z_closure_sample_check(this_: &z_owned_closure_sample_t) -> bool {
    !this_.is_empty()
}
