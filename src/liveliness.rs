...
#[no_mangle]
pub extern "C" fn _zc_liveliness_token_null(this_: &mut MaybeUninit<zc_owned_liveliness_token_t>) {
    this_.as_rust_type_mut_uninit().write(None);
}

/// Returns ``true`` if liveliness token is valid, ``false`` otherwise.
#[no_mangle]
pub extern "C" fn _zc_liveliness_token_check(this_: &zc_owned_liveliness_token_t) -> bool {
