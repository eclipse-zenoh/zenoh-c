...
use crate::{
    ...
    z_owned_string_t, z_string_copy_from_substr, _z_string_null,
};
...
#[no_mangle]
pub extern "C" fn _z_config_null(this_: &mut MaybeUninit<z_owned_config_t>) {
...
pub unsafe extern "C" fn zc_config_get_from_substr(
    ...
    if key.is_null() {
        _z_string_null(out_value_string);
...
#[no_mangle]
#[allow(clippy::missing_safety_doc)]
pub extern "C" fn _z_config_check(this_: &z_owned_config_t) -> bool {
...
pub unsafe extern "C" fn zc_config_to_string(
    ...
    match json5::to_string(config) {
        ...
        Err(e) => {
            ...
            _z_string_null(out_config_string);
            result::Z_EPARSE
        }
    }
}
