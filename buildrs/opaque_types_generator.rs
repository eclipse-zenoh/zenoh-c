...
fn copy_cargo_files_to_out_dir(cargo_lock: &Path) -> PathBuf {
    ...
    std::fs::copy(cargo_lock, &dest_lock).unwrap_or_else(|_| {
        panic!(
...
pub fn generate_opaque_types(path_out: &Path, target: &str, cargo_lock: &Path) {
    let type_to_inner_field_name = HashMap::from([("z_id_t", "pub id")]);
    let manifest_path = copy_cargo_files_to_out_dir(cargo_lock);
