...
///
/// Input: map of target triple -> path to probe_build_output.txt (as produced by build_probe_project).
/// Output: HashMap<type_name, HashMap<target, (size, alignment)>>
///
/// Behavior:
...
pub fn parse_probe_resutls(
    outputs: &HashMap<String, std::path::PathBuf>,
    ...
    for (target, path) in outputs.iter() {
        let data = std::fs::read_to_string(path).unwrap_or_else(|e| {
            panic!(
                "Failed to read probe output for target {target} at {}: {e}",
                path.display()
            )
        });
        ...
        if let Some(first) = data.lines().find(|l| !l.trim().is_empty()) {
            if first.trim_start().starts_with("error:") {
                panic!(
                    "Cargo error encountered while building probe for target {target}.\nPath: {}\n\nOutput:\n{}",
                    path.display(),
                    data
                );
            }
        ...
        if total_error_count == 0 {
            panic!(
                "No Rust errors found in probe output for target {target}.\nPath: {}\n\nOutput:\n{}",
                path.display(),
                data
            );
        }
        ...
        for cap in re_sizes.captures_iter(&data) {
            ...
            map.entry(type_name)
                .or_default()
                .insert(target.clone(), (size, align));
        }

        if matched_count == 0 {
            panic!(
                "Probe output for target {target} contains errors but none matched size/alignment pattern.\nPath: {}\n\nOutput:\n{}",
                path.display(),
                data
            );
        }
        ...
        if matched_count != total_error_count {
            panic!(
                "Mismatch in probe output for target {target}: found {total_error_count} errors but matched {matched_count} size/alignment entries.\nPath: {}\n\nOutput:\n{}",
                path.display(),
                data
            );
        }
