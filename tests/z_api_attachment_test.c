...
void writting_through_map_by_alias_read_by_get() {
   ...
   z_loaned_bytes_t attachment = z_slice_map_as_attachment(&map);
    ...
    z_loaned_slice_t a1 = z_attachment_get(attachment, z_slice_from_str("k1"));
    ASSERT_STR_BYTES_EQUAL("v1", a1);

    z_loaned_slice_t a2 = z_attachment_get(attachment, z_slice_from_str("k2"));
    ASSERT_STR_BYTES_EQUAL("v2", a2);

    z_loaned_slice_t a_non = z_attachment_get(attachment, z_slice_from_str("k_non"));
...
int8_t _attachment_reader(z_loaned_slice_t key, z_loaned_slice_t value, void* ctx) {
...
void writting_no_map_read_by_get() {
   z_loaned_bytes_t attachment = {.data = NULL, .iteration_driver = &_iteration_driver};
