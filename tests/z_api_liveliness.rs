...
use zenoh_ffi::{
    ...
    z_loaned_sample_t, z_open, z_owned_closure_reply_t, z_owned_config_t,
    z_owned_liveliness_token_t, z_owned_reply_t, z_owned_session_t, z_reply_drop, z_reply_is_ok,
    z_reply_loan, z_reply_move, z_reply_ok, z_sample_keyexpr, z_sample_kind, z_sample_kind_t,
    z_session_drop, z_session_loan, z_session_move, z_sleep_s, z_string_data, z_string_len,
    z_subscriber_drop, z_subscriber_move, z_view_keyexpr_from_str, z_view_keyexpr_loan,
    z_view_keyexpr_t, z_view_string_loan, z_view_string_t, Z_CHANNEL_DISCONNECTED, Z_OK,
};
...
extern "C" fn on_receive(s: &mut z_loaned_sample_t, context: *mut c_void) {
    unsafe {
        ...
        // z_view_string_t ks;
        let mut ks = MaybeUninit::<zenoh_ffi::z_view_string_t>::uninit();
...
fn liveliness_sub() {
    unsafe {
        ...
        let mut closure = MaybeUninit::<zenoh_ffi::z_owned_closure_sample_t>::uninit();
        ...
        let mut sub = MaybeUninit::<zenoh_ffi::z_owned_subscriber_t>::uninit();
...
fn liveliness_get() {
    unsafe {
        ...
        let mut handler = MaybeUninit::<zenoh_ffi::z_owned_fifo_handler_reply_t>::uninit();
